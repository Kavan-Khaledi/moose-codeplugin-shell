...
#include "ADStressDivergenceTriangularShell.h"
...
registerMooseObject(MOOSEAPPNAME, ADStressDivergenceTriangularShell);

InputParameters
ADStressDivergenceTriangularShell::validParams()
{
...
ADReal
ADStressDivergenceTriangularShell::computeQpResidual()
{
  ...
  for (_qp_z = 0; _qp_z < _t_weights.size(); ++_qp_z)
  {
    ...
    if (_large_strain)
      residual1 +=
          ((*_stress_old[_qp_z])[_qp](0, 0) * (*_B_nl[_qp_z])[_qp](0, _i + _component * 6) +
           (*_stress_old[_qp_z])[_qp](1, 1) * (*_B_nl[_qp_z])[_qp](1, _i + _component * 6) +
           2.0 * (*_stress_old[_qp_z])[_qp](0, 1) * (*_B_nl[_qp_z])[_qp](2, _i + _component * 6) +
           2.0 * (*_stress_old[_qp_z])[_qp](0, 2) * (*_B_nl[_qp_z])[_qp](3, _i + _component * 6) +
           2.0 * (*_stress_old[_qp_z])[_qp](1, 2) * (*_B_nl[_qp_z])[_qp](4, _i + _component * 6));
